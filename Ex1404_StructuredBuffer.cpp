...
bool Ex1404_StructuredBuffer::Initialize() {
    ...
    std::uniform_real_distribution<float> dc(0.2f, 1.f);
...
void Ex1404_StructuredBuffer::Update(float dt) {
    ...
    m_constsCPU.deltaTime = dt;

    for (auto &p : m_particles.m_cpu) {
        p.position += Vector3(-p.position.y, p.position.x, 0.0f) * dt;
        //p.position += Vector3(0.3f);
        //p.color = Vector3(dc(gen), dc(gen), dc(gen));
    }

	//m_particles.Upload(m_context);
    //m_particles.Download(m_context);


    D3D11Utils::UpdateBuffer(m_context, m_constsCPU,
                                                  m_constsGPU);
}
...
void Ex1404_StructuredBuffer::Render() {
	...
	float controlPoint = 1024.f;
     m_context->Dispatch(UINT(ceil(m_particles.m_cpu.size() / controlPoint)), 1, 1);
...
void Ex1404_StructuredBuffer::UpdateGUI() {
    // ImGui::SliderFloat("Scale", &m_constsCPU.deltaTime, 0.0f, 120.f);
}
