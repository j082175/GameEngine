...
void AppBase::Render() {
    ...
    vector<ID3D11ShaderResourceView *> commonSRVs(4);
    if (m_skyboxCheck == 0) {
        commonSRVs = {m_SRVs.m_envSRV.Get(), m_SRVs.m_specularSRV.Get(),
                      m_SRVs.m_irradianceSRV.Get(), m_SRVs.m_brdfSRV.Get()};
    } else if (m_skyboxCheck == 1) {
        commonSRVs = {m_SRVs2.m_envSRV.Get(), m_SRVs2.m_specularSRV.Get(),
                      m_SRVs2.m_irradianceSRV.Get(), m_SRVs2.m_brdfSRV.Get()};
    }
...
void AppBase::InitCubemaps(wstring basePath, wstring envFilename,
    ...
    D3D11Utils::CreateDDSTexture(
        m_device, (basePath + specularFilename).c_str(), true, specularSRV);
    D3D11Utils::CreateDDSTexture(
        m_device, (basePath + irradianceFilename).c_str(), true, irradianceSRV);
    D3D11Utils::CreateDDSTexture(m_device, (basePath + brdfFilename).c_str(),
                                 false, brdfSRV);

    if (num == 0) {
        m_SRVs.m_envSRV = envSRV;
        m_SRVs.m_irradianceSRV = irradianceSRV;
        m_SRVs.m_specularSRV = specularSRV;
        m_SRVs.m_brdfSRV = brdfSRV;
    } else if (num == 1) {
        m_SRVs2.m_envSRV = envSRV;
        m_SRVs2.m_irradianceSRV = irradianceSRV;
        m_SRVs2.m_specularSRV = specularSRV;
        m_SRVs2.m_brdfSRV = brdfSRV;
    }
}
