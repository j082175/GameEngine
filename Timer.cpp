...
namespace hlab {
hlab::Timer::Timer(ComPtr<ID3D11Device> &device) {
...
void hlab::Timer::Start(ComPtr<ID3D11DeviceContext> &context, bool measureGPU) {
...
void hlab::Timer::End(ComPtr<ID3D11DeviceContext> &context) {
    ...
    if (m_measureGPU) {
        ...
        while (context->GetData(m_disjointQuery.Get(), NULL, 0, 0) == S_FALSE)
            continue;
        D3D11_QUERY_DATA_TIMESTAMP_DISJOINT tsDisjoint;
        context->GetData(m_disjointQuery.Get(), &tsDisjoint, sizeof(tsDisjoint),
                         0);
        ...
        if (!tsDisjoint.Disjoint) {
            ...
            context->GetData(m_startQuery.Get(), &startTime, sizeof(UINT64), 0);
            context->GetData(m_stopQuery.Get(), &stopTime, sizeof(UINT64), 0);

            m_elapsedTimeGPU =
                (double(stopTime - startTime) / double(tsDisjoint.Frequency)) *
                1000.0;
        } else {
    ...
    m_elapsedTimeCPU =
        (std::chrono::high_resolution_clock::now() - m_startTimeCPU).count() /
