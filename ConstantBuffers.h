...
inline void ConstantBuffer<T_CONSTS>::Initialize(ComPtr<ID3D11Device> &device) {
    D3D11Utils::CreateConstBuffer(device, m_cpu, m_gpu);
}

template <typename T_CONSTS>
inline void
ConstantBuffer<T_CONSTS>::Upload(ComPtr<ID3D11DeviceContext> &context) {
    D3D11Utils::UpdateBuffer(context, m_cpu, m_gpu);
}

template <typename T_CONSTS>
inline T_CONSTS &ConstantBuffer<T_CONSTS>::GetCpu() {
    return m_cpu;
}

template <typename T_CONSTS> inline const auto ConstantBuffer<T_CONSTS>::Get() {
    return m_gpu.Get();
}

template <typename T_CONSTS>
inline const auto ConstantBuffer<T_CONSTS>::GetAddressOf() {
    return m_gpu.GetAddressOf();
}
