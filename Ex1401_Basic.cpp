...
bool Ex1401_Basic::Initialize() {
    ...
    m_device->CreateTexture2D(&desc, NULL, texture.GetAddressOf());
    ...
    ThrowIfFailed(m_device->CreateUnorderedAccessView(
        backBuffer.Get(), NULL, m_backUAV.GetAddressOf()));
...
void Ex1401_Basic::Render() {
    ...
    m_context->Dispatch(UINT(ceil(m_screenWidth / 32.f)), ceil(m_screenHeight / 32.f), 1);
