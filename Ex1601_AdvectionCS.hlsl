          ...
          uint3 dtID : SV_DispatchThreadID)
{
	...
	float2 vel = velocityTemp.SampleLevel(pointWrapSS, pos, 0.f).xy;
	...
	velocity[dtID.xy] = velocityTemp.SampleLevel(linearWrapSS, posBack, 0.f);
	density[dtID.xy] = densityTemp.SampleLevel(linearWrapSS, posBack, 0.f);
